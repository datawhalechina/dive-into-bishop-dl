...
$$
很粗糙的说，单连通是指这个集合中的任意两点之间都存在一条路径（也就是这个集合可以被视为“一片”，非单连通的集合例如两个分开的圆盘就不能视作“一片”了），读者不难发现凸性比单连通要强很多，因此凸性蕴含了决策区域是单连通的。我们可以轻易地证明这件事。假设 $\boldsymbol{x}_{1}, \boldsymbol{x}_{2} \in \mathcal{R}_{k}$ 考虑这两点的判别函数
$$
\begin{align*}
