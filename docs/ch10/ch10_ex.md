...
当我们固定只讨论输出的有效位置即核完全落在图像内时， $j=L-1,...,0$ 且 $k=M-1,...,0$ ，这使得上式中的求和可以直接写为 $\ell=0\dots L-1, m=0\dots M-1$ ，因为此时 $j-\ell$ 和 $k-m$ 总在图像内。
...
把左边的坐标偏移掉（或把输出下标理解为 $j',k'$），我们就得到了与互相关形式数值上等价的表达。
