...
考虑使用一个 $n$ 维特征向量预测目标值的问题。我们有数据集 $\{ (\boldsymbol{x}_{n}, t_{n}) \}_{n=1}^{N}$，定义函数
$$
y(\boldsymbol{x}, \boldsymbol{w}) = w_{0} + w_{1}x_{1} + \cdots + w_{D}x_{D} \tag{Lin}
$$
其中 $\boldsymbol{x} \in \mathbb{R}^{D}$，$\boldsymbol{w} \in \mathbb{R}^{D+1}$. 注意上面的函数是一个线性映射，因此这样的问题也叫**线性回归**。
...
![](https://files.mdnice.com/user/55419/64ad30f9-9997-4d97-8f3f-a2502b5d0417.png)
...
\end{align*}
$$
其中 $\displaystyle \sigma(x) = \frac{1}{1 + \exp(-x)}$ 是 Sigmoid 函数。


![image](https://files.mdnice.com/user/55419/e28f1764-aa17-4312-bf90-553fcc75131f.png)

<center>不同基函数的曲线。从左至右依次是多项式、Gaussian和Sigmoid基函数</center>
...
![image](https://files.mdnice.com/user/55419/adb5d33c-fddf-46e3-bf05-d87faba4f44f.png)
